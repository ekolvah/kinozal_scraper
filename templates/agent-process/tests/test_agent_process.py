...
_IMPLEMENTER_CONTRACT_MARKERS = (
    ...
    # decision through the one gate with an exit code.
    "scripts.review_gate",
    "`not ready`",
)
...
_PLANNER_RUNBOOK_MARKERS = (
    ...
    # Codex skill when the fourth one landed.
...
# sources" with every test green, which is the drift this tuple exists to catch.
...
# The observation bounds the planner runbook carried until . They move wholesale to
...
# interface fact, while `SHOULD-FIX` reproduces the severity taxonomy.
_SHARED_GATE_DEFINITIONS = (
    ("SHOULD-FIX", "docs/architecture/agent-process.md"),
    ("NICE-TO-HAVE", "docs/architecture/agent-process.md"),
    # Preventive scope for the Claude implementer shim, green on add: both
    ...
    # `.claude/commands/implement.md` because the fat 25-line copy removed in
    ...
    # feeds it: both are contract, so no adapter may carry either.
    (
    ...
    # adapter may invoke the role, never re-decide how far the observation goes.
...
class TestAgentProcess:
    ...
    def test_documented_control_plane_caps_match_the_catalogue(self) -> None:
        ...
        # hardcoded list goes green on it.
    ...
    def test_documented_carrier_selection_modes_match_the_catalogue(self) -> None:
        """A selection mode nobody documented is a rule only the validator knows."""
        process = (_REPO / "docs" / "architecture" / "agent-process.md").read_text(encoding="utf-8")
        catalogue = yaml.safe_load(
            (_REPO / ".agents" / "orchestration" / "roles.yaml").read_text(encoding="utf-8")
        )
    ...
    def test_every_codex_skill_is_a_finished_adapter(self) -> None:
        """Both Codex entry points, not only the first one that was written.
    ...
    def test_planner_adapters_share_the_canonical_runbook(self) -> None:
        ...
        flow` is canon and `implement-issue/SKILL.md` points at it.
        """
        process = (_REPO / "docs" / "architecture" / "agent-process.md").read_text(encoding="utf-8")
    ...
    def test_discovery_role_declares_carriers_for_the_evidence_contract(self) -> None:
        """The `## Evidence` block has a role that owes it, not just a section.
    ...
    def test_planner_runbook_delegates_the_observation_to_discovery(self) -> None:
        """The bounds move; they are not copied into a second home.
    ...
    def test_planner_marks_planned_and_no_implementer_home_unmarks_it(self) -> None:
        """The board flag is planner-only, in the canon and nowhere else.
    ...
    def test_documented_verdict_words_come_from_the_validator(self) -> None:
        """The accepted verdicts have one home, the way the change-class matrix does.
    ...
    def test_every_declared_role_adapter_resolves_to_its_contract(self) -> None:
        """Adapter coverage stays symmetric per role, or the gap is named.
    ...
    def test_provider_specific_adapter_files_do_not_define_shared_gates(self) -> None:
        ...
        a definition is deleted rather than moved.
        """
    ...
    def test_architect_reviewer_declares_independence_for_every_adapter(self) -> None:
        """Whether a carrier reviews its own plan is catalogue data, not prose.
    ...
    def test_review_fix_loop_stop_rule_lives_only_in_the_gate(self) -> None:
        """The condition failed twice as prose; it gets one home now."""
        homes = {
            ...
            # reading of the findings, which is the / recurrence.
    ...
    def test_review_outcome_enforcement_is_documented_without_a_path_exception(self) -> None:
        """: the controller-PR carve-out is gone; one contract applies to all."""
        process = (_REPO / "docs" / "architecture" / "agent-process.md").read_text(encoding="utf-8")
    ...
    def test_mindset_names_the_red_to_green_recovery_recipe(self) -> None:
        """: the second compaction lands at the RED->GREEN boundary, not exploration.
