...
1. **RED first** — падающий тест из issue `## Test plan` пишется до кода
   ...
   Форму теста под гейт подгонять не нужно: `scripts/check_red.py` берёт исход **на тест**
   из junit-отчёта, поэтому `unittest.subTest` его не ломает (#400; до фикса тест, у которого
   падали все сабтесты, засчитывался зелёным, и гейт диктовал разбивать его на отдельные).
