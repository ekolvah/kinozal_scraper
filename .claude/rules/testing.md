...
1. **RED first** — падающий тест из issue `## Test plan` пишется до кода
   ...
   `scripts/check_red.py` берёт исход **на тест** из junit-отчёта, поэтому `unittest.subTest`
   его не ломает и дробить параметризованный тест ради гейта не надо (#400). Но одно
   требование к форме RED-коммита есть: набор, который **не выполнился** (не собрался, упал
