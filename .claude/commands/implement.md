...
5. **Docs**: обнови `.md` из `## Docs to update`. Если сознательно отклонил покрытие (scope-/cost-skip, live-E2E как negative-ROI) — запиши это в ledger `docs/architecture/coverage-gaps.md` руками; автогенерации инвентаря тестов нет. Обоснование решения в state-док не разворачивай: маршрут — `project-map.md` §Canonical-home (архитектурное решение с высокой ценой разворота → запись MADR в `docs/adr/` по `template.md`), в доке остаётся ссылка.
