...
6. **Gate**: `python scripts/ci_check.py` — **одним foreground-вызовом с `timeout: 600000`**, не в фоне и без цикла ожидания: прогон идёт минуты (§Среда в `CLAUDE.md`), и polling обходится дороже самого прогона. Red → root cause, не workaround. `.githooks/pre-push` всё равно повторит этот gate.
