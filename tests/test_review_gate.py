"""Tests for the review/fix loop stop gate (#467).
...
class TestVerdict:
    def test_green_required_checks_are_ready_for_human(self) -> None:
        """Round 2 of #465: no blocking finding, two should-fix — the loop ends."""
    ...
    def test_exhausted_fixer_budget_escalates(self) -> None:
        """Round 4 of #465: three revisions spent, so no fifth round is routed."""
        evidence = _evidence(
            head_sha=_ROUND_4,
...
class TestEvidence:
    ...
    def test_broken_capture_exits_two(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """`stdout is None` means the reader died, not an empty answer (#364/#410)."""
