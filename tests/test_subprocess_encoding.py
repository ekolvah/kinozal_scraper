...
на памяти автора, по канону `principles.md` (§Scripts over instructions) крепится exit-code'ом.
...
def _imports_subprocess_by_name(source: str) -> list[str]:
    ...
    не заслуживает того, чтобы быть записанной прозой (`principles.md`)."""
    return [
        alias.name
        for node in ast.walk(ast.parse(source))
