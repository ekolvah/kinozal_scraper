...
каждой новой формой записи.

**Скоуп — индекс git минус `docs/adr/`.** `git ls-files`, а не обход ФС: `.claude/worktrees/`
gitignored и держит копии репо со старыми доками (тот же аргумент, что в
`tests/test_doc_links.py`). Записи MADR исключены **по жанру**: запись — дом обоснования,
она датирована по конструкции и после принятия иммутабельна, так что покрасневший на ней
...
может отставать на единицу**, если выше в том же абзаце code-span перенесён через строку:
CommonMark заменяет перевод строки внутри `` ` `` пробелом, и в `content` его уже нет.
...
def _tracked_docs() -> tuple[str, ...]:
    """Отслеживаемые `.md` вне `docs/adr/`. `-z`: иначе git C-квотит нелатиницу."""
    ...
    names = [name for name in result.stdout.split("\0") if name.endswith(".md")]
