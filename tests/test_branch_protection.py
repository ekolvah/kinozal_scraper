"""Guards around required status checks for the `main` branch (#436).

Actual enforcement lives in GitHub configuration **outside the repository**: before #436, the `pr-link`
...
class TestDriftDetection:
    ...
    def test_missing_required_context_is_drift(self) -> None:
        """The exact #436 defect: `pr-link` is declared but absent from GitHub."""
...
class TestProtectionFetch:
    ...
    def test_none_stdout_exits_2(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Broken output capture (#364/#410) is also infrastructure failure, not an empty response."""
...
class TestPrePushHook:
    ...
    def test_failing_gate_is_not_rerun_under_the_next_interpreter(self, tmp_path: Path) -> None:
        """A red gate is a verdict, not “the interpreter is wrong” (root cause #436)."""
        self._stub(tmp_path / ".venv" / "Scripts" / "python", "scripts")
    ...
    def test_missing_interpreter_fails_loudly(self, tmp_path: Path) -> None:
        ...
        **Preservation guard**: today’s hook already has this property (a missing interpreter makes
        `bash` fail with a nonzero code), so the test is not in the #436 RED set—it pins what
