...
- **Y. Always-load статус `mindset.md` гардом не стережётся (#416).** Правка §(2)
  ...
  **Исполнено в #375** — ровно в оговорённой форме, на весь каталог:
  `tests/test_always_load_budget.py::test_expected_files_are_in_scope` краснеет, когда
  любой из трёх файлов выпадает из набора (в т.ч. из-за появившегося `paths:`).
  Возражение «у `mindset.md` нет frontmatter» снято тем, что гард утверждает **членство
  в always-load наборе**, а не наличие/отсутствие метаданных, — добавление frontmatter
  без `paths:` его не красит.
