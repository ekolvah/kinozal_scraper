...
- **X. Кодировка subprocess: гард стережёт сторону родителя, не ребёнка (#364).**
  ...
  заведомо ASCII. **Также не строится** унификация девяти call-site через общий
  `run_text()`-хелпер: она устраняет повторяющийся аргумент ценой рефакторинга
  девяти файлов, тогда как гард закрывает тот же класс диффом в три строки (§VII);
  кандидат в отдельную issue, если call-site станет заметно больше. И **отклонён
  `PYTHONUTF8=1` как единственное лекарство** — он чинит обе половины сразу, но
  живёт в состоянии среды, невидим на свежем клоне и не защищает call-site,
  запущенный иначе; гарантия слабее, чем у гейта на исходнике. Записано, чтобы
  «а почему не хелпер / не переменная среды» не переоткрыли как work-for-work.
