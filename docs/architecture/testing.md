...
tool/rule Y" → [`ci.md` §Consciously not adopted](ci.md#consciously-not-adopted), which is where
that split is stated in full (#419).
...
- **Y. Always-load статус `mindset.md` — ЗАКРЫТО в #375 (был пробел #416).** #416 добавлял
  тактики в файл, обязанный грузиться в каждой сессии, — появление `paths:` во frontmatter
  молча выключило бы их, а гарда не было. Тогда изобретать его отвергли: `mindset.md`
  frontmatter-блока не имел вовсе, то есть гард сторожил бы отсутствие того, чего не
  существует, и краснел бы от любого добавления метаданных, а не от потери always-load.
  Оговорка была — «если понадобится, писать сразу на весь каталог, а не на один файл»;
  **#375 так и сделал**: `tests/test_always_load_budget.py::test_expected_files_are_in_scope`
  краснеет, когда любой из трёх файлов выпадает из набора (в т.ч. из-за появившегося
  `paths:`). Прежнее возражение снято формой утверждения — гард проверяет **членство в
  always-load наборе**, а не наличие метаданных, поэтому frontmatter без `paths:` его не
  красит. Оставлено в списке как след решения: «а почему нет теста на always-load» больше
  не вопрос, но путь от «отвергнуто» к «сделано в оговорённой форме» — тот случай, ради
  которого ledger и ведётся.
...
- **AA. «Док не должен снова разрастись» гейтом не стережётся (#419).** Свёртка `ci.md`
  ...
  work-for-work. **Граница действия этой записи (#375):** она про `docs/architecture/*` —
  доки, которые читаются по требованию, где размер лишь *прокси* качества. Для
  **always-load** набора гейт наоборот заведён (`tests/test_always_load_budget.py`), и это
  не противоречие: там байты — не прокси, а сама плата, взимаемая с каждой сессии, а порог
  работает **храповиком**, не нормативом качества (он не утверждает «текст хороший» — он
  делает рост платы осознанной правкой одной строки на ревью). Различает эти два случая
  именно вопрос «метрика — прокси или сама стоимость», а не «размер вообще гейтить нельзя».
...
- **AB. Бюджет always-load меряет у́же сессионной преамбулы (#375).** `test_always_load_budget`
  ...
  `description:` сабагентов и слэш-команд и индекс `MEMORY.md`. Расширять скоуп здесь
  отвергнуто: у этих носителей другой механизм загрузки (по вызову / отдельный индекс), и
  один порог на разнородную сумму мешал бы диагностику — красный тест не сказал бы, где
  вырос. Плата за решение честная: **cost-shifting туда гейт не поймает**, ровно как не
  поймает перенос текста в `docs/architecture/*`, который агент всё равно читает по
  требованию. Revisit-триггер: если очередной прирост платы обнаружится именно в
  agent/command-frontmatter — заводить **второй** счётчик, а не расширять этот.

- **AC. Протухание model-specific утверждений в `mindset.md` §(2) гейтом не стережётся (#375).**
  Секция несёт калиброванные под текущую модель факты («дефолтная длина у Opus 5 выше прежней»,
  «делегирует охотнее предыдущих») со ссылкой на migration guide. Строка «при смене модели
  перепроверять там же» — формально та самая форма «надо не забыть про X», которую блок
  «Скрипты > инструкции» в том же файле объявляет триггером к гейту, и гейта здесь **нет**
  сознательно: детерминируемая часть — «в репе есть model-claims», а протухли они или нет,
  решается чтением внешнего дока, то есть суждением. Гард на присутствие фразы был бы зелёным
  ровно пока текст на месте — **независимо от того, верен ли он**: ложное покрытие (§IV), тот же
  класс, что детектор семантических дублей. Настоящий триггер перепроверки — **смена пина
  модели**, а он уже загейчен: `tests/test_agent_frontmatter.py` пинит модели сабагентов, и
  правка пина физически проходит через ревью. Revisit-триггер: если очередной бамп модели
  оставит §(2) устаревшей на несколько PR — вешать проверку на **пин**, а не на прозу.
