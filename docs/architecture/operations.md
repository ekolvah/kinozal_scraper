...
| `KINOZAL_URLS` | var | Kinozal listing URLs selected by the operator, format `label\|url;...`; a listing may contain mixed item categories. Local fallback — environment variable `KINOZAL_TOP_URL` (plain URL). If neither is set, the pipeline logs `no URLs configured`. The legacy name `URLS` is **no longer read** (clean rename, #263). `sources.json` `url`/`base_url` is **not read** for scraping (schema placeholder only); see `kinozal_pipeline.py::_kinozal_urls` |
| `KINOZAL_EXCLUDED_ITEM_CATEGORIES` | var | **Optional.** `;`-separated, case-insensitive denylist of readable per-item Kinozal categories from `details.php`, with whitespace normalized. An exact name such as `Другое - АудиоКниги` denies that category; a group such as `Музыка` denies every `Музыка - ...` category. Denied items are not notified or sent to trailer lookup, but are saved to Sheets for dedup. Empty/unset disables this filter and logs INFO. Recommended production value after the fix reaches `main`: `Музыка;Другое - АудиоКниги;Другое - Библиотека;Другое - Программы;Другое - Дизайн / Графика;Другое - Видеоклипы`; games are deliberately allowed. The obsolete feed-selector variable `KINOZAL_EXCLUDED_CATEGORIES` must not be used. See [`pipeline.md` § Kinozal item-category filtering](pipeline.md#kinozal-item-category-filtering) (#506) |
| `KINOZAL_EXCLUDED_GENRES` | var | **Optional.** Independent `;`-separated denylist of genres (case-insensitive), e.g. `Hidden objects`. A new item whose details-page genre is in the list is not notified, but is saved to Sheets for dedup. Empty/unset disables the genre filter. The shared details pass runs once per new item when either this denylist or `KINOZAL_EXCLUDED_ITEM_CATEGORIES` is non-empty; when both are empty it makes no details requests. See `kinozal_pipeline.py::_apply_item_filters` (#263, #506) |
| `KINOZAL_USERNAME` | secret | **Optional.** Account login for the `kinozal.guru` mirror — enables automatic fallback to the mirror when the primary host named by `KINOZAL_URLS` fails (522, or the challenge gate). What is enabled and how links change — [`pipeline.md` § Kinozal mirror fallback](pipeline.md#kinozal-mirror-fallback). Paired with `KINOZAL_PASSWORD`; **partial** (only one of the two) → WARNING + fallback disabled (not failure) |
...
cost proxy. The 100,000 raw-token/turn floor was calibrated on 2026-08-21 from
