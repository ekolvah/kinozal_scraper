...
| `KINOZAL_URLS` | var | Kinozal page URLs to scrape, format `label\|url;...`. Production film pages use `t=1`. The explicit denylist is mixed `t=0`, music `t=4`/`41–44`, library `t=5`, audiobooks `t=6`, and programs `t=8`. Plain denied IDs stop before fetch; otherwise the category selected in the response is authoritative, catching missing/malformed/currently unknown values that Kinozal maps to `t=0`. A future integer category that the site recognises is processed. Missing response-category markup fails open with a WARNING. Denials make the source visibly non-ok while sibling URLs continue (#506); see the [listing-category guard](pipeline.md#trailer-retrieval-and-selection) for exact behavior. Local fallback `KINOZAL_TOP_URL` follows the same rule. If neither variable is set, the pipeline logs `no URLs configured`. The legacy name `URLS` is **no longer read** (clean rename, #263). `sources.json` `url`/`base_url` is **not read** for scraping (schema placeholder only); see `kinozal_pipeline.py::_kinozal_urls` |
| `KINOZAL_EXCLUDED_GENRES` | var | **Optional.** `;`-separated denylist of details-page `Жанр` values (case-insensitive), e.g. `Hidden objects`. A new item whose genre is in the list is **not** notified, but is saved to Sheets (dedup). Empty/unset → filter disabled; details pages are not requested (zero overhead). This is independent from the `KINOZAL_URLS` listing-category denylist: genres such as `Фантастика` can belong to both films and audiobooks and therefore cannot identify content type safely. See `kinozal_pipeline.py::_split_by_excluded_genre` (#263, #506) |
