...
- **AK. Второй носитель review-гейта не проверен живым прогоном (#478).** Failover на
  `openai/codex-action` покрыт структурно (`TestFallbackCarrier`: порядок шагов, условие запуска,
  гейт на секрет, bracket-синтаксис `final-message`, атрибуция продюсера, красный гейт при
  пропущенном носителе) — то есть **workflow описан как задумано**, но что второй носитель
  реально возвращает структурированный вердикт, гардами не доказано: шаг не стартует без
  `secrets.OPENAI_API_KEY`, а это платный ключ, и его выдача — решение владельца, не побочный
  эффект мержа. **Пропуск сознательный, не тихий:** отсутствие ключа даёт ровно сегодняшнее
  поведение (нет вердикта → красный `claude-review`), то есть непроверенная ветка не может
  ослабить гейт — она может только не сработать, и это видно как красная проверка, а не как
  зелёный PR без ревью. Класс тот же, что у **AD** (сетевая половина branch-protection): цена
  покрытия — долгоживущий секрет и расход, компенсация — оффлайн-гард на in-repo половину.
  **Триггер закрытия — первый прогон с заданным ключом**: тогда ссылка на run попадает в
  `## Agent record` issue, и эта запись снимается. Решение целиком —
