...
| `scripts/check_otel_event_delivery.py` | Operator-invoked, read-only, thresholdless check that Claude Code events reach Loki: reads both signals over one window through the Grafana datasource proxy and exits non-zero when sessions appear in metrics and no event series does. Why the repository may hold live credentials at all: ADR-0010; operation is in `operations.md#verify-and-import`; the coverage it moved is `coverage-gaps.md` §AN (#542) |
