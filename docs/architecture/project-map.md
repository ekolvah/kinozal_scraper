...
[`testing.md`](testing.md#consciously-accepted-coverage-gaps).
...
- **Операционные процедурные правила** (workflow) → в `.claude/rules/` **целиком — правило и
  ...
  **Rationale ≠ нарратив** (#375) — always-load-файл несёт первое, но не второе:
  - **остаётся:** решение + одна фраза «почему оно до сих пор верно» (без неё правило выглядит
    ритуалом и его снесёт следующий «упрощатель»);
  - **уезжает:** нарратив «как мы к этому пришли» — даты, номера коммитов, что именно поймало
    ревью. Его дом — тело issue/PR, в правиле остаётся голый `(#N)`. Пересказывать GitHub в репо
    значит держать копию, которая будет дрейфовать;
  - **отвергнутая альтернатива**, которую нельзя переоткрывать → ledger в
    [`testing.md`](testing.md#consciously-accepted-coverage-gaps), а не в always-load.

  Перед удалением нарратива — **проверить `gh issue view N`, что он там действительно есть**:
  часть фактов бывает артефактом сессии и на GitHub не попала (прецедент — #150).
