...
| `scripts/eval_trailers.py` | Eval-harness подбора трейлера: dual-скоркарта `TrailerStrategy` (YouTube) + `evaluate_tmdb` (TMDB-источник) по frozen golden-set (Hit/Wrong/Miss относительно `correct`, офлайн) + `--record`/`--record-tmdb`; deep-dive `testing.md#eval-harness--trailer-selection-139` (#139, #329) |
