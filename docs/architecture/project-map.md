...
`.claude/rules/`. Вне — файлы, чей frontmatter несёт `description:` (`.claude/agents/*.md`,
`.claude/commands/*.md`): у них header живёт **там**, и граница проведена **по свойству файла**,
а не перечислением каталогов, чтобы следующий агент или команда не попали в серую зону
(обратный случай — `.claude/rules/testing.md`: frontmatter есть, `description:` нет, значит
строка-маркер обязательна). `CLAUDE.md` исключён **сознательно**: он тонкий роутер, помеченный
в «Карте файлов» ❌ kitchen-sink, и требовать от него единственного вопроса значило бы
зафиксировать гейтом ту роль, от которой его надо освобождать.
...
**есть с чем спорить** о границе файла, — но не что header соответствует содержимому. Расхождение docstring ↔ реальное назначение ловит человек
