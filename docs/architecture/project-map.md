...
переименование секции иначе молча рвёт все входящие якоря. Их **форму** стережёт
`tests/test_doc_narrative.py` (#428): `#N` — скобочный указатель, не член предложения, и в
...
- **Идеи, задачи, roadmap, нереализованные инициативы → GitHub issues** (они переживают переезд на
  ...
  трейлер-инициативы в `docs/initiatives/` отклонили, scope распределён по issue (#188, #138–#145).
...
- **Сигил `#` зарезервирован за ссылками на issue/PR.** Номер правила — `` `workflow.md §7` ``,
  ...
  который растёт с каждой новой формой записи.
