...
| `agent-process-export.md` | Which files of the agentic-process contract can move to a new project as-is, templated, or not at all; its citation, link, and exported-payload-size policy; canonical entry point, with [ADR-0011](../adr/0011-agentic-process-distribution-mechanism.md), for the tracked copier seed | ❌ manifest + 3 policies |
...
| `templates/agent-process/` + `tests/test_agent_process_template.py` | Tracked one-shot Copier seed for the Layer 0 + Codex Layer 2 payload. `copier.yml` supplies project answers, template-only suffixes, exclusions, and install-order notice; the test parses the export manifest for parity and renders default plus representative alternate payloads to guard escaped f-string braces, Jinja branches, citation/prose/link stripping, missing test references, generated Python quality, and the 30 KB architecture-doc budget |
