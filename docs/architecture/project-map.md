...
| `templates/agent-process/` + `tests/test_agent_process_template.py` | Tracked one-shot Copier seed for the Layer 0 + Codex Layer 2 payload. `copier.yml` supplies project answers, template-only suffixes, exclusions, and install-order notice; the test parses the export manifest for parity and renders a default payload to guard escaped f-string braces, citation/link stripping, and the 30 KB architecture-doc budget |
