...
| `scripts/verify_pr_link.py` | CI-гейт (job `pr-link` в `ci.yml`): PR из `issue-N` ветки обязан закрывать issue, иначе job red → required check блокирует мёрдж. Агент-независимый backstop к `open_pr.py` (переиспользует его чистые функции); enforcement через gate, не прозу (#320) |
...
| Подбор трейлера (retrieval → selection) | `src/kinozal_scraper/youtube.py` (retrieval: `search_candidates` — union RU+оригинал → `list[Candidate]`, #140), `src/kinozal_scraper/kinozal_pipeline.py` (`build_film_profile` — data-prep `FilmProfile` с details.php, #140), `src/kinozal_scraper/trailer_strategy.py` (selection: `FilmProfile`/`Candidate`/`TrailerPick` + `TrailerStrategy` Protocol + baseline `FirstResultStrategy`, #139) | `pipeline.md#trailer-retrieval--film-profile-140` · `testing.md#eval-harness--trailer-selection-139` |
