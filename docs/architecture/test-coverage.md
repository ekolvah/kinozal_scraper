...
| D. Config errors | `test_pipeline_config.py::TestValidateSourcesConfig` (incl. `test_invalid_css_row_selector_raises`, `test_invalid_css_field_selector_raises`, `test_broken_at_selector_rejects_css_part`, `test_valid_selectors_with_attr_pass`, `test_unresolved_macro_in_url_raises_config_error`, `test_unresolved_macro_in_nested_field_raises`), `TestExpandMacros`, `TestBuildMacroContext`, `TestLoadSourcesConfig`; `test_generic_pipeline.py::TestSelectorCssPart` | ✅ — invalid CSS in `row_selector` / `dedupe_key` / `fields.*` of html sources now raises `ConfigError` at load time ([#57](https://github.com/ekolvah/kinozal_scraper/issues/57)); unresolved `{{macro}}` also raises `ConfigError` at load time ([#58](https://github.com/ekolvah/kinozal_scraper/issues/58)) |
