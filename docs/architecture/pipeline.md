...
**Бюджет запросов на прогон (#384).** Суточная квота YouTube — **100 `search.list`**
(замер 2026-07-26 через Service Usage API; квота дефолтная, billing выключен, поднять нельзя).
Фильм стоит ≤2 запроса, поэтому обогащение обрывается после `_TRAILER_RUN_BUDGET = 45` фильмов:
остальные **не ходят в сеть вообще** и несут третий §IV-маркер `⚠️ трейлер: дневная квота YouTube`
(не промах и не поломка — другая причина, другое действие оператора). Оператор видит **одну**
WARNING-строку с числом необслуженных фильмов, а не строку на каждый.
...
retry отбирал бы бюджет у следующего фильма. Штатный потолок конфига (4 URL × `limit: 50` = 200
...
**безвредные**. Любое изменение логики отбора обязано проходить через
