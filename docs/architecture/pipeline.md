...
| `{trailer_url}` | raw YouTube trailer URL, or a §IV miss/failure/low-confidence marker (see below) |
| `{trailer_link}` | `<a href="{trailer_url}">Trailer</a>` — clickable "Trailer" word for an http(s) URL; a non-http value (a §IV marker `🎬 трейлер не найден` on a clean miss / `⚠️ трейлер: ошибка поиска` on a lookup failure, #138 / the `⚠️ трейлер неточный: <url>` **prefix** on a low-confidence pick, #359) renders as visible escaped text; empty only when `trailer_url` is unset (non-kinozal sources) |
...
INFO-breadcrumb `reason`/`confidence`. Успешный pick с `confidence < _TRAILER_MIN_CONFIDENCE`
(0.5; сегодня туда попадает только ambiguous-ничья 0.3) отдаётся под префиксом
`⚠️ трейлер неточный: ` — стратегия признаёт монетку, и это обязано дойти до пользователя,
а не остаться в логе (#359: `enrich_with_trailer` раньше выбрасывал `confidence`). URL
сохраняется (Miss стоил бы recall'а: равноранговые кандидаты обычно настоящие трейлеры), но
кликабельность держится на авто-детекте plain-URL клиентом Telegram — строка с префиксом не
проходит `startswith("http")` и рендерится текстом. Ничьи в проде частые (5/6 picks в run
`30066249488`), потому что прод-профиль без каста схлопывает ключ ранжирования — root cause
частоты вынесен в **#377**, порог под него не подкручивать. **Gemini НЕ в hot path** — LLM(#142)/embeddings(#143)/
...
(RU-приоритет на языке заголовка; per-item details-фетч ради каст-тай-брейка отложен — **#377**,
там же замер цены: при непустом `KINOZAL_EXCLUDED_GENRES` details-HTML уже тянется на каждый item).
