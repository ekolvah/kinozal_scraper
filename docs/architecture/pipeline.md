...
| `{trailer_link}` | `<a href="{trailer_url}">Trailer</a>` — clickable "Trailer" word for an http(s) URL; a non-http value (a §IV marker `🎬 трейлер не найден` on a clean miss **or a low-confidence pick** #359 / `⚠️ трейлер: ошибка поиска` on a lookup failure, #138) renders as visible escaped text; empty only when `trailer_url` is unset (non-kinozal sources) |
...
(0.5; сегодня туда попадает только ambiguous-ничья 0.3) отдаётся как **miss-маркер, без
ссылки** (#359): стратегия признаёт монетку, а ссылка, которую пользователь всё равно обязан
проверить сам, стоит меньше честного маркера — та же преференция, что уже зашита в скоринг
харнесса (`Hit +1 / Miss 0 / Wrong −2`). До #359 `enrich_with_trailer` читал только
`video_id is None` и выбрасывал `confidence`, поэтому коин-флип уходил неотличимо от
уверенного pick'а. Ничьи в проде частые (5/6 picks в run `30066249488`), потому что
прод-профиль без каста схлопывает ключ ранжирования — ссылки возвращает **#377** (снимает
причину ничьих), а НЕ понижение порога. **Gemini НЕ в hot path** — LLM(#142)/embeddings(#143)/
