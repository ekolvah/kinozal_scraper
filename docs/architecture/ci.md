...
**Куда попадает решение — первое совпадение** (канон правила — [`project-map.md`](project-map.md)
§Canonical-home): «не покрыли тестом X» → [`coverage-gaps.md`](coverage-gaps.md); «не взяли
инструмент или правило Y» → **сюда** (либо строкой по месту своего гейта, если у него есть секция
выше); «архитектурное решение с высокой ценой разворота, задевающее несколько модулей или доков» →
запись в [`docs/adr/`](../adr/0001-record-architecture-decisions.md); всё остальное записью не
становится — его дом остаётся телом issue/PR. Эта секция — **только** для инструмента целиком,
у которого своей секции-гейта нет.
