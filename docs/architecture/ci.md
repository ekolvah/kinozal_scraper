...
- **У каждого правила — anti-drift-гард**, пинящий код в *effective select* и вне глобального
  `ignore`: `tests/test_complexity_ratchet.py`, `test_ruff_dead_code_rule.py`,
  `test_ruff_arg_slf_rules.py`, `test_ruff_docstring_rule.py` (все — в жанре
  `test_ruff_silence_rules.py`). Третья ветка — `per-file-ignores` — у них **двух форм, и форма
  следует из наличия легитимного исключения**: `ARG`/`SLF` проверяются по покрытию путей
  (`fnmatch` против sentinel-путей `src`/`scripts`), потому что `tests/**` им заглушён законно;
  у `ERA001` и docstring-кодов легитимных исключений нет, поэтому проверка строже — кода не
  должно быть в `per-file-ignores` **ни под каким паттерном** (иначе узкий возврат вида
  `"tests/test_x.py" = ["D100"]` прошёл бы мимо sentinel-зонда).
