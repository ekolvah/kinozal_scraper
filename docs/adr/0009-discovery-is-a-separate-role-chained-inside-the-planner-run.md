...
Since #509 a `bug` issue must carry a completed observation of the external system before its plan
...
the same class of invisibility that #474 (self-review indistinguishable from independent review) and
#473 (a role covered by one provider and not another) already fixed elsewhere.

The architect review of #517 pushed back on the answer, not the diagnosis: the two concrete defects
...
* Bad, because the provenance line would resolve against the planner's own carrier set, so a planner
  attesting to its own observation is the #474 defect re-created in a new section.
...
* Naming: the catalogue key stays `discovery` rather than `discoverer`/`observer`, so it matches the
  section-family name the issue contract already uses; recorded as declined in #517 `## Out of scope`.
