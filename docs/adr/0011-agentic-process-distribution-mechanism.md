...
so the actual export (#560's own follow-up) does not reopen it.
...
* Overlaps with #550 (audit replacing bespoke repository scripts with
  ...
  distributed. Not blocked by #550 — a later replacement flows to the
  vendored copy through the same `copier update` as any other change.
* Conflicts with #309 (an MCP server over the dev scripts — one install
  instead of a per-project copy) on distribution mechanism. Not resolved
  here: #309 must reconcile with this choice when it is implemented, not
