...
## PR Workflow
- **Новый bug/feature → `/plan #N` → `/implement #N`.** Ручной workflow ниже — только если `/implement` упал на escape hatch или задача тривиальна (опечатка, однострочный non-behavioural твик, docs-only PR). Команды живут в `.claude/commands/{plan,implement}.md`, контракт sections — в `.github/ISSUE_TEMPLATE/*.yml` + `.github/pull_request_template.md`. Rationale: issue #114.
...
- Предпочтительно `/commit-push-pr` из плагина `commit-commands` — авто-ветка если на main (для ручного fallback'а).
