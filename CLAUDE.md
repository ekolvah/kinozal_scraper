...
- [Project map](docs/architecture/project-map.md) — **полное оглавление навигации**: какой файл на какой вопрос отвечает (процесс + исходники + все deep-dive arch-доки runtime/pipeline/storage/testing/test-coverage/ci/gemini) + где живёт какое знание (tier-модель + canonical-home). Отдельные доки сюда поштучно **не дублируем** — спускаемся через этот индекс.
