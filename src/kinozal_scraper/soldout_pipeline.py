...
# Dedicated source type (grain of github_popular's `github_popular`), not the
# former format-keyed shared `html` bucket that kinozal/github_trending also use.
# A generic multi-source runner is deferred until ≥2 event sources with a uniform
# single-GET fetch actually exist (#276).
_SOURCE_TYPE = "soldout"


def run_soldout_pipeline(
    storage: Storage,
    notifier: Notifier,
    ...
    if not soldout_sources:
        logger.info("no enabled '%s' source found", _SOURCE_TYPE)
        return results

    for source in soldout_sources:
        try:
            result = _run_single_source(source, storage, notifier)
...
def _run_single_source(
    ...
    if not url:
        logger.warning("[%s] no URL configured (set SOLDOUT_URL)", source_id)
        return result
